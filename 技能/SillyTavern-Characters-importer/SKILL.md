...
1. **第一步：解析 JSON 并立即保存**
   - PNG 输入 → 读取二进制 → 解析 chunk → 提取 JSON（base64 解码）
   - JSON 输入 → 直接读取
   - **立即保存** `设定/角色卡原始数据.json` ← 这是所有后续步骤的数据源

2. **确定故事名：** 从刚保存的 JSON 的 `name` 字段读取（**不询问用户**）
   - 检查 `故事/rp-{name}/` 是否已存在
   - 如果存在 → 追加后缀 `-1`、`-2`……直到不冲突

3. **智能分类提取**
   - 按分类规则将 entries 分配到各文件：
     `设定/角色.md` `设定/世界背景.md` `设定/开场白.md` `设定/风格指南.md` `设定/主角.md` `设定/状态变量.md`
   - 开场白 `first_mes` + `mes_example` → `开场白.md`

4. **兜底二次匹配**
   - 对空分类执行 `fallback_classify()`
...
> ⚠️ 不要在此步骤输出"构建故事"的引导——root AGENTS.md 已自动串联 Characters-to-world。
> 提取完成后，直接返回结果给主对话，主对话会自动继续构建。
